...
[`crates/aequitas-python`](crates/aequitas-python) publishes this surface as
the `aequitas-python` wheel, importable as `pyaequitas`. The law crate stays
`no_std` and never depends on `pyo3`; the binding is the only place the two
meet.

Dimensions here are type parameters and are erased at codegen, so the binding
exports a runtime image instead: a base-unit magnitude carried with the seven
SI exponents and a semantic discriminant. Both are derived from the type
parameters, and every unit symbol and scale is read from the `LinearUnit`
associated constants, so a Python conversion cannot disagree with a Rust one.
Dimensional errors `rustc` rejects at compile time raise there instead.

The inventory and the type stubs are generated by
[`scripts/generate-surface.py`](scripts/generate-surface.py); CI runs its
`check` mode, so a quantity added here that never reached the binding fails the
gate. The decision and its alternatives are in
